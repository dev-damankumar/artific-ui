...
describe('Render Avatar Default Themes', () => {
    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            const {container} = render(
                <Avatar theme={theme}>Click Me</Avatar>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                theme === 'default' ? 'avatar' : `avatar-${theme}`,
            )
            expect(component).not.toHaveClass(
                ...except(themes, theme === 'default' ? '' : `avatar-${theme}`),
            )
        })
    })
...
describe('Render Avatar Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render ${layout} layout`, () => {
            const {container} = render(
                <Avatar layout={layout}>Click Me</Avatar>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                layout === 'default' ? 'avatar' : `avatar-${layout}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    layouts,
                    layout === 'default' ? '' : `avatar-${layout}`,
                ),
            )
        })
    })
...
describe('Render Avatar Variants', () => {
    ...
    variants.forEach(variant => {
        test(`render ${variant} variant`, () => {
            const {container} = render(
                <Avatar variant={variant}>Click Me</Avatar>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                variant === 'default' ? 'avatar' : `avatar-${variant}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    variants,
                    variant === 'default' ? '' : `avatar-${variant}`,
                ),
            )
        })
    })
...
describe('Render Avatar Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            ...
            const component = container.firstChild
            expect(component).toHaveClass(
                size === 'md' ? 'avatar' : `avatar-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(sizes, size === 'md' ? '' : `avatar-${ISizes[size]}`),
            )
        })
    })
})
