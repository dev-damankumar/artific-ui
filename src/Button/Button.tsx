...
export const Button: React.FC<Omit<IButton, 'prefix'> & IButtonProps> = (
    ...
    const additionalVariantClasses = `${
        variant === 'text' && `${componentSelector}-outline`
    }`
    const loadingClasses =
        (loading &&
            `${
                hideTextOnLoading && `${componentSelector}-loading-no-text`
            } ${componentSelector}-loading ${
                loadingDirection === 'after' &&
                `${componentSelector}-loading-right`
            } ${
                loadingStyle === 'grow' && `${componentSelector}-loading-grow`
            }`) ||
        ''
    ...
    return (
        <>
                ...
                onMouseDown={(e: React.MouseEvent<HTMLButtonElement>) => {
                    ...
                    if (rippleEvents.mouseDown) {
                        if (
                            ...
                            const rippleEl =
                                button.querySelector('[data-ripple]')
            ...
            >
                ...
                <span
                    className={getClassNames(styles, 'btn-ripple')}
                    data-ripple={true}
                ></span>
            </button>
        </>
