...
function ripple(e: any, button: any, rippleEl: any, focus: any, config: any) {
    ...
    const ripple_elmnt: any = document.createElement('span')
    const diameter =
        Math.max(parseInt(style.height), parseInt(style.width)) * 1.5
    ...
    ripple_elmnt.style.transition = `transform ${ms}ms ease, opacity ${
        ms - 100
    }ms ease`
    ripple_elmnt.style.background = `${Color(backgroundColor)
        .hsl()
        .darken(0.4)
        .alpha(0.3)
        .string()}`
