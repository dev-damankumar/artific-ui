        ...
        const {container} = render(
            <Accordion>
                <AccordionPanel>
                    ...
                    <AccordionBody>Header 1 Content</AccordionBody>
                </AccordionPanel>
                <AccordionPanel>
                    <AccordionHeader>Header 2</AccordionHeader>
                    <AccordionBody>Header 2 Content</AccordionBody>
                </AccordionPanel>
            </Accordion>,
        )
...
describe('Render Accordion Default Themes', () => {
    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            ...
            expect(component).toHaveClass(
                theme === 'default' ? 'accordion-wrap' : `accordion-${theme}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    themes,
                    theme === 'default' ? '' : `accordion-${theme}`,
                ),
            )
        })
    })
    test('render custom color scheme', () => {
        ...
        const {container} = render(
            <Accordion data-testid="accordion" colorScheme={colorScheme}>
                <AccordionHeader>Header 1</AccordionHeader>
                <AccordionBody>Header 1 Content</AccordionBody>
            </Accordion>,
        )
...
describe('Render Accordion Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render ${layout} layout`, () => {
            ...
            const component = container.firstChild
            expect(component).toHaveClass(
                layout === 'default' ? 'accordion-wrap' : `accordion-${layout}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    layouts,
...
describe('Render Accordion Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            ...
            const component = container.firstChild
            expect(component).toHaveClass(
                size === 'md' ? 'accordion-wrap' : `accordion-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    sizes,
                    size === 'md' ? '' : `accordion-${ISizes[size]}`,
                ),
            )
        })
    })
...
describe('Render Accordion With Indicator Positions', () => {
    test('render start position', () => {
        const {container} = render(
            <Accordion data-testid="accordion" indicatorPosition="start">
                <AccordionHeader>Header 1</AccordionHeader>
                <AccordionBody>Header 1 Content</AccordionBody>
            </Accordion>,
        )
    ...
    test('render end position', () => {
        const {container} = render(
            <Accordion data-testid="accordion" indicatorPosition="end">
                <AccordionHeader>Header 1</AccordionHeader>
                <AccordionBody>Header 1 Content</AccordionBody>
            </Accordion>,
        )
