...
describe('Render Card', () => {
    ...
    test('render card content', () => {
        const {container} = render(
            <Card>
                <CardContent data-testid="content">This is content</CardContent>
            </Card>,
        )
...
describe('Render Card Default Themes', () => {
    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            ...
            expect(component).toHaveClass(
                theme === 'default' ? 'card' : `card-${theme}`,
            )
            expect(component).not.toHaveClass(
                ...except(themes, theme === 'default' ? '' : `card-${theme}`),
            )
        })
    })
    test('render custom color scheme', () => {
        ...
        const {container} = render(
            <Card colorScheme={colorScheme}>Click Me</Card>,
        )
...
describe('Render Card Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render ${layout} layout`, () => {
            ...
            const component = container.firstChild
            expect(component).toHaveClass(
                layout === 'default' ? 'card' : `card-${layout}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    layouts,
...
describe('Render Card Variants', () => {
    ...
    variants.forEach(variant => {
        test(`render ${variant} variant`, () => {
            const {container} = render(
                <Card variant={variant}>Click Me</Card>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
...
describe('Render Card Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            ...
            const component = container.firstChild
            expect(component).toHaveClass(
                size === 'md' ? 'card' : `card-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(sizes, size === 'md' ? '' : `card-${ISizes[size]}`),
            )
        })
    })
})
...
describe('Render Card Image Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render card image ${layout} layout`, async () => {
            ...
            expect(content).toHaveClass(
                layout === 'default' ? 'card-image' : `card-image-${layout}`,
            )
            expect(content).not.toHaveClass(
                ...except(
                    layouts,
                    layout === 'default' ? '' : `card-image-${layout}`,
                ),
            )
        })
    })
    ...
    test('render card image extended layout', async () => {
        ...
        expect(content).not.toHaveClass(
            ...except(layouts, 'card-image-no-radius'),
        )
    })
