...
const assignClasses = (conditions: LoaderConfig) => {
    let loaderClasses = ''
    if (conditions.variant !== 'default')
        loaderClasses += ` ati-loader-${conditions.variant}`
    if (conditions.theme !== 'dark')
        loaderClasses += ` ati-loader-${conditions.theme}`

    return loaderClasses
}
export const useLoader = (function () {
    function Constructor(
        this: ILoader & LoaderConfig & { selector: Element | null },
        loadStyles = true,
    ) {
        ...
        function createLoader(
            ...
            const sizeClass =
                this.size !== 'md' ? `ati-loader-${this.size}` : ''
            const loader = Loader(
                this.style || '',
                `${this.className} ${componentId} ${classes}`,
                sizeClass,
            )
            if (loadStyles) {
            ...
            const prevLoader = document.querySelector(
                `[data-ati-loader-modal].${componentId}`,
            )
            if (!prevLoader) {
