...
export const Modal: React.FC<IModalProps & IDiv> = (
    ...
    const additionalModalSize = OverLaySizesArray.includes(size || '')
        ? sizeClass
        : ''
    const additionalModalContentSize = !OverLaySizesArray.includes(size || '')
        ? sizeClass
        : ''
    ...
    const onBackdropEffect = (e: React.MouseEvent<HTMLElement>) => {
        ...
        (e.target as HTMLElement)?.firstElementChild?.classList.add(
            getClassNames(modalStyles, 'shake').trim(),
        )
        setTimeout(() => {
            (e.target as HTMLElement).firstElementChild?.classList.remove(
                getClassNames(modalStyles, 'shake').trim(),
            )
        }, 500)
    ...
    return (
        <>
            ...
            >
                <div
                    style={styles}
                    className={`${classes} ${getClassNames(
                        modalStyles,
                        'modal-content',
                        additionalModalContentSize,
                    )}`}
                >
                    {childrenWithProps}
                </div>
