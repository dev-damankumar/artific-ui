...
describe('Render Pagination Default Themes', () => {
    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            const {container} = render(
                <Pagination theme={theme}>Click Me</Pagination>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                theme === 'default' ? 'pagination' : `pagination-${theme}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    themes,
                    theme === 'default' ? '' : `pagination-${theme}`,
                ),
            )
        })
    })
    test('render custom color scheme', () => {
        ...
        const {container} = render(
            <Pagination colorScheme={colorScheme}>Click Me</Pagination>,
        )
...
describe('Render Pagination Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render ${layout} layout`, () => {
            const {container} = render(
                <Pagination layout={layout}>Click Me</Pagination>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                layout === 'default' ? 'pagination' : `pagination-${layout}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    layouts,
                    layout === 'default' ? '' : `pagination-${layout}`,
                ),
            )
        })
    })
...
describe('Render Pagination Variants', () => {
    ...
    variants.forEach(variant => {
        test(`render ${variant} variant`, () => {
            const {container} = render(
                <Pagination variant={variant}>Click Me</Pagination>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                variant === 'default' ? 'pagination' : `pagination-${variant}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    variants,
...
describe('Render Pagination Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            const {container} = render(
                <Pagination size={size}>Click Me</Pagination>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                size === 'md' ? 'pagination' : `pagination-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    sizes,
                    size === 'md' ? '' : `pagination-${ISizes[size]}`,
                ),
            )
        })
    })
