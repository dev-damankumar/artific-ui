    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            ...
            expect(component).toHaveClass(
                theme === 'default' ? 'table' : `table-${theme}`,
            )
            expect(component).not.toHaveClass(
                ...except(themes, theme === 'default' ? '' : `table-${theme}`),
            )
        })
    })
...
describe('Render Table Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render ${layout} layout`, () => {
            ...
            const component = getByRole(container, 'table')
            expect(component).toHaveClass(
                layout === 'default' ? 'table' : `table-${layout}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    layouts,
                    layout === 'default' ? '' : `table-${layout}`,
                ),
            )
        })
    })
...
describe('Render Table Variants', () => {
    ...
    variants.forEach(variant => {
        test(`render ${variant} variant`, () => {
            ...
            const component = getByRole(container, 'table')
            expect(component).toHaveClass(
                variant === 'default' ? 'table' : `table-${variant}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    variants,
...
describe('Render Table Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            ...
            const component = getByRole(container, 'table')
            expect(component).toHaveClass(
                size === 'md' ? 'table' : `table-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(sizes, size === 'md' ? '' : `table-${ISizes[size]}`),
            )
        })
    })
})
