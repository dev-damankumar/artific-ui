...
describe('Render Dropdown Default Themes', () => {
    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            const {container} = render(
                <Dropdown theme={theme}>Click Me</Dropdown>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                theme === 'default' ? 'dropdown' : `dropdown-${theme}`,
            )
    ...
    test('render custom color scheme', () => {
        ...
        const {container} = render(
            <Dropdown colorScheme={colorScheme}>Click Me</Dropdown>,
        )
...
describe('Render Dropdown Layouts', () => {
    ...
    layouts.forEach(layout => {
        test(`render ${layout} layout`, () => {
            const {container} = render(
                <Dropdown layout={layout}>Click Me</Dropdown>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                layout === 'default' ? 'dropdown' : `dropdown-${layout}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    layouts,
                    layout === 'default' ? '' : `dropdown-${layout}`,
                ),
            )
        })
    })
...
describe('Render Dropdown Variants', () => {
    ...
    variants.forEach(variant => {
        test(`render ${variant} variant`, () => {
            const {container} = render(
                <Dropdown variant={variant}>Click Me</Dropdown>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                variant === 'default' ? 'dropdown' : `dropdown-${variant}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    variants,
...
describe('Render Dropdown Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            const {container} = render(
                <Dropdown size={size}>Click Me</Dropdown>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                size === 'md' ? 'dropdown' : `dropdown-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    sizes,
                    size === 'md' ? '' : `dropdown-${ISizes[size]}`,
                ),
            )
        })
    })
...
describe('Render Dropdown Note Variant Directions', () => {
    ...
    directions.forEach(direction => {
        test(`render ${direction} direction`, () => {
            const {container} = render(
                <Dropdown position={direction}>Click Me</Dropdown>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                direction === 'bottom' ? 'dropdown' : `drop-${direction}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    directions,
                    direction === 'left' ? '' : `drop-${direction}`,
                ),
            )
        })
    })
