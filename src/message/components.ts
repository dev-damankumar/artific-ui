import {IconType, MessageConfig, MessageType, Types} from "../types/message";
...
export const Header = (type: Types | undefined, header: string) => {
	if (type === 'notification') {
		return `<h3 class="ati-message-header">${header ? header : (type as string).toUpperCase()}</h3>`
	}
	return ''
}
...
export const MessageAlert = (msg: string, header: string, cond: MessageConfig & { msgType: MessageType }) => {
