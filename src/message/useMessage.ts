import {ColorScheme, FunctionCallback, Layouts, Sizes, Themes} from "../types/common";
...
type MessageType = 'success' | 'info' | 'error' | 'warning' | 'alert'
type Positions = 'top-left' | 'top' | 'top-right' | 'bottom-left' | 'bottom' | 'bottom-right'
...
export const useMessage = (function () {
	...
	function Constructor(this: Messages & MessageConfig) {
		this.type = 'default';
		this.mode = 'light';
		...
		const makeMessage = (type: MessageType, header: string, msg: string) => {
			...
			const styleCss = `<style>
					${customCss?.()?.props?.children}
			</style>`
			const classes = `${componentSelector} ${componentId} ${this.className} 
			${[(this.colorScheme || type === 'alert') ? `${componentSelector}-${this.theme}` : '',
				layoutClasses,
				variantClasses,
				this.type === 'notification' ? 'ati-message-alert-notification' : '',
				sizeClass].join(' ')}`
			...
			div.className = `ati-message-alert-destroying ${this.mode === 'dark' ? 'ati-mode-dark' : ''} ${this.dismissible ? 'ati-message-alert-dismissible' : ''} ${classes} ${(type && type !== 'alert') ? `ati-message-${type}` : ''}`
			div.innerHTML = `
							${this.dismissible ? `<button type="button" class="ati-message-close">
        <span class="ati-message-close-icon">&times;</span>
      </button>` : ''}
                              ${type !== 'alert' ? `<img src="${icons[type]}">` : `${icons[type]}`}
                              <div class="ati-message-message-wrap">
                              ${this.type === 'notification' ? ` <h3 class="ati-message-header">${header ? header : type.toUpperCase()}</h3>` : ''}
                                
                                 <p class="ati-message-message">${msg}</p>
                              </div>`
			if (this.dismissible) {
		...
		function render(this: MessageConfig, toast: HTMLElement) {
			let wrapperExist = document.querySelector(`[data-ati-message-wrapper]${this.position !== 'top-right' ? `.ati-message-alert-position-${this.position}` : ''}`)
			if (wrapperExist) {
				wrapperExist.className = `${this.position !== 'top-right' ? `ati-message-alert-position-${this.position}` : ''} ati-message-alert-wrapper`
				...
				wrapper.className = `${this.position !== 'top-right' ? `ati-message-alert-position-${this.position}` : ''} ati-message-alert-wrapper`
			...
			timeHandler = setTimeout(setTimeForDestroy.bind(this, toast), (this.duration * 1000) + delay * 400)
			delay++
		}

		this.success = (message: string, header: string) => {
			let toast = makeMessage('success', header, message);
			render.call(this, toast)
		...
		this.alert = (message: string, header: string) => {
			let toast = makeMessage('alert', header, message);
			render.call(this, toast)
			return this as Messages
		}
	...
	return function instance() {
		return (new (Constructor as any)() as Messages)
	}
})()
