...
const assignClassesToAlert = (
    conditions: MessageConfig & {
        msgType: MessageType
    ...
    if (conditions.msgType && conditions.msgType !== 'alert')
        alertClasses += ` ati-message-${conditions.msgType}`
    if (conditions.colorScheme || conditions.msgType === 'alert')
        alertClasses += ` ${conditions.componentSelector}-${conditions.theme}`
    if (conditions.type === 'notification')
        alertClasses += ' ati-message-alert-notification'
    return alertClasses
}
...
export const useMessage = (function () {
    function Constructor(
        ...
        loadStyles = true,
        themeProvider: IThemeProvider | null | undefined,
    ) {
        ...
        const isStyleAppied = document.querySelector(
            '[data-ati-message-alert-style]',
        )
        ...
        const makeMessage = (
            ...
            const variantClasses =
                this.variant !== 'default'
                    ? `${componentSelector}-${this.variant}`
                    : ''
            const layoutClasses =
                this.layout !== 'default'
                    ? `${componentSelector}-${this.layout}`
                    : ''
            const sizeClass = sizeClasses(componentSelector, this.size)
            const customCss: any = applyColorScheme(
                componentSelector,
                this.colorScheme,
                componentId,
            )
            ...
            if (this.dismissible) {
                div?.querySelector('.ati-message-close')?.addEventListener(
                    'click',
                    () => {
                        const el = div.querySelector('.ati-message-close')?.parentElement
                        if (el) {
                            destroyMessage.call(
                                this,
                                el
        ...
        function render(this: MessageConfig, toast: HTMLElement) {
            const isThemeStyleExist = document.querySelector(
                `[data-artific-css='${themeId}']`,
            )
            const wrapperExist = document.querySelector(
            ...
            if (wrapperExist) {
                ...
                if (!isThemeStyleExist) {
                    if (themeProvider?.theme) {
                        ...
                        const themeStyles = `<style data-artific-css=${themeId}>${
                            themeStyle && themeStyle
                        }</style>`
                        wrapperExist.insertAdjacentHTML(
                            'afterbegin',
                            themeStyles,
                        )
                    }
            ...
            timeHandler = setTimeout(
                setTimeForDestroy.bind(this, toast),
                (this.duration || 1) * 1000 + delay * removingSeconds,
            )
            delay++
        }
    ...
    return function instance(
        this: Messages & MessageConfig,
        loadStyle = true,
        themeProvider: IThemeProvider | null | undefined = null,
    ) {
