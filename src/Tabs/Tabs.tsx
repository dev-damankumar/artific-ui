...
export const Tabs: React.FC<ITabsProps & IDiv> = ({
    ...
    const lineDirectionClasses =
        variant === 'floating-line'
            ? `${componentSelector}-${variant}-${lineDirection}`
            : ''
    ...
    React.Children.forEach(children, (child: React.ReactNode) => {
        if (React.isValidElement(child)) {
            if (
                child.props?.originalType?.displayName === 'TabContent' ||
                (child.type as React.FC)?.displayName === 'TabContent'
            ) {
                tabContents.push(
                    React.cloneElement<any>(child, {
                        variant,
                        activeTab,
                        key: getRandomClassId(),
                    }),
                )
            } else {
