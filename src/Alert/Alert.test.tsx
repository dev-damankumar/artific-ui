...
describe("Render Alert Default Themes", () => {
	...
	themes.forEach((theme) => {
		test(`render ${theme} theme`, () => {
			...
			const component = container.firstChild;
			expect(component).toHaveClass(theme === 'default' ? 'alert' : `alert-${theme}`)
			expect(component).not.toHaveClass(...except(themes, theme === 'default' ? '' : `alert-${theme}`))
		});
	})
...
describe("Render Alert Layouts", () => {
	...
	layouts.forEach((layout) => {
		test(`render ${layout} layout`, () => {
			const {container} = render(<Alert layout={layout}>Click Me</Alert>);
			const component = container.firstChild;
			expect(component).toHaveClass(layout === 'default' ? 'alert' : `alert-${layout}`)
			expect(component).not.toHaveClass(...except(layouts, layout === 'default' ? '' : `alert-${layout}`))
		});
	})
});
...
describe("Render Alert Variants", () => {
	...
	variants.forEach((variant) => {
		test(`render ${variant} variant`, () => {
			const {container} = render(<Alert variant={variant}>Click Me</Alert>);
			const component = container.firstChild;
			expect(component).toHaveClass(variant === 'default' ? 'alert' : `alert-${variant}`)
			expect(component).not.toHaveClass(...except(variants, variant === 'default' ? '' : `alert-${variant}`))
		});
	})
});
...
describe("Render Alert Sizes", () => {
	...
	sizes.forEach((size) => {
		test(`render ${size} size`, () => {
			...
			const component = container.firstChild;
			expect(component).toHaveClass(size === 'md' ? 'alert' : `alert-${ISizes[size]}`)
			expect(component).not.toHaveClass(...except(sizes, size === 'md' ? '' : `alert-${ISizes[size]}`))
		});
	})
});
...
describe("Render Alert Note Variant Directions", () => {
	...
	directions.forEach(direction => {
		test(`render ${direction} direction`, () => {
			const {container} = render(<Alert variant="note" direction={direction}>Click Me</Alert>);
			const component = container.firstChild;
			expect(component).toHaveClass(direction === 'left' ? 'alert' : `alert-note-${direction}`)
			expect(component).not.toHaveClass(...except(directions, direction === 'left' ? '' : `alert-note-${direction}`))
		});
	})
});
