...
export const Alert: React.FC<IAlertProps & IDiv> = (
    ...
    const {customCss, classNames} = getDefaultClasses(
        styles,
        componentSelector,
        className,
        theme,
    ...
    const additionalVariantClasses = `${
        variant === 'note' && direction && direction !== 'left'
            ? `${componentSelector}-note-${direction}`
            : ''
    } ${variant === 'note' ? 'alert-outline' : ''}`
    return show ? (
        <>
            {customCss && customCss()}
            <div
            ...
            >
                {dismiss && (
                    <button
                        ...
                        type="button"
                        className={getClassNames(styles, 'close')}
                    >
                        &times;
                    </button>
                )}
