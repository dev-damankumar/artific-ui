...
describe('Render Spinner', () => {
    test('render the Spinner component', () => {
        const {container} = render(
            <Spinner data-testid="spinner">Hello World!</Spinner>,
        )
...
describe('Render Spinner Default Themes', () => {
    ...
    themes.forEach(theme => {
        test(`render ${theme} theme`, () => {
            const {container} = render(
                <Spinner theme={theme}>Click Me</Spinner>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                theme === 'default' ? 'spinner' : `spinner-${theme}`,
            )
    ...
    test('render custom color scheme', () => {
        ...
        const {container} = render(
            <Spinner data-testid="spinner" colorScheme={colorScheme}>
                Click Me
            </Spinner>,
        )
        const spinner = getByTestId(container, 'spinner')
...
describe('Render Spinner Variants', () => {
    ...
    variants.forEach(variant => {
        test(`render ${variant} variant`, () => {
            const {container} = render(
                <Spinner variant={variant}>Click Me</Spinner>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                variant === 'default' ? 'spinner' : `spinner-${variant}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    variants,
...
describe('Render Spinner Sizes', () => {
    ...
    sizes.forEach(size => {
        test(`render ${size} size`, () => {
            const {container} = render(
                <Spinner size={size}>Click Me</Spinner>,
            )
            const component = container.firstChild
            expect(component).toHaveClass(
                size === 'md' ? 'spinner' : `spinner-${ISizes[size]}`,
            )
            expect(component).not.toHaveClass(
                ...except(
                    sizes,
                    size === 'md' ? '' : `spinner-${ISizes[size]}`,
                ),
            )
        })
    })
