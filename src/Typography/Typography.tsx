...
export const Typography: React.FC<ITypographyProps & IAnyElement> = (
    ...
    const element =
        variant === 'abbr' ? 'abbr' : variant === 'pre' ? 'pre' : variant === 'highlight' || variant === 'kbd'
            ? 'span'
            : variant === 'code'
                ? 'code'
                : asEl;
    ...
    const weightClasses = `${
        weight !== 'medium' ? `typography-${weight}` : ''
    }`;
    const additionalThemeClass = !colorScheme
        ? theme !== 'default'
            ? `text-${theme}`
            : ``
        : ``;
    ...
    const mainElement = createElement(element, {
        ...
        } ${getClassNames(classes, additionalThemeClass)} ${getClassNames(
            classes,
            'typography',
            variantClasses,
            weightClasses,
