	...
	useEffect(() => {
		if (scrollRef.current) {
			scrollRef.current.addEventListener('scroll', (e) => {
				...
				const pageScrolledPercentAge=Math.trunc((currentScroll / (totalScroll - scrollRef.current.offsetHeight)) * 100)
				setProgress(pageScrolledPercentAge)
			})
		}
	}, [])
	return <AppContext.Provider value={{mode: mode, setMode: toggleThemeMode, progress: progress}}>
		<div className={`container-div ${mode==='dark' ? 'dark-mode-wrapper' : ''}`}>
